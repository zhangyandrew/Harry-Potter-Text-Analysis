...
```{r}
mayorsfoodcourt <- read.csv("~/Desktop/mayorsfoodcourt.csv")
```
...
same_food_court <- locat1 %>% group_by(locat1$LICENSENO)
sum<-same_food_court %>% summarise(sum(same_food_court$pass_fail))
View(sum)
# score <- act_Court %>% group_by(businessName,ViolStatus) %>%
#   summarise(businessName,ViolStatus)
